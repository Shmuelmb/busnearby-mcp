...
describe("DirectionsAPI", () => {
  ...
  it("should construct the correct URL with basic parameters", async () => {
    ...
    expect(url.searchParams.get("arriveBy")).toBe(
      String(defaultParams.arriveBy)
    )
    expect(url.searchParams.get("locale")).toBe(defaultParams.locale)
    expect(url.searchParams.get("wheelchair")).toBe(
      String(defaultParams.wheelchair)
    )
    expect(url.searchParams.get("mode")).toBe(defaultParams.mode)
    expect(url.searchParams.get("showIntermediateStops")).toBe(
      String(defaultParams.showIntermediateStops)
    )
    expect(url.searchParams.get("numItineraries")).toBe(
      String(defaultParams.numItineraries)
    )
    expect(url.searchParams.get("maxWalkDistance")).toBe(
      defaultParams.maxWalkDistance
    )
    expect(url.searchParams.get("optimize")).toBe(defaultParams.optimize)
    expect(url.searchParams.get("ignoreRealtimeUpdates")).toBe(
      String(defaultParams.ignoreRealtimeUpdates)
    )
